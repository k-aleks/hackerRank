...
namespace hackerRank
{
    ...
    internal class BinaryTreeIndexer_Tests
    {
        ...
        public void Test_Log2()
        {
            for (int i = 1; i < 100; i++)
            {
                Console.Out.WriteLine($"i={i}, log={(int)Math.Log(i, 2)}");
                indexer.LogTwo(i).Should().Be((int)Math.Log(i, 2));
            }
        }

        [Test]
        public void Test_Log_and_Log2_Comparison()
        {
            for (int i = 1; i < 100; i++)
            {
                Console.Out.WriteLine($"i={i}, log2={(int)Math.Log(i, 2)}, log={(int)Math.Log(i)}");
            }
        }
